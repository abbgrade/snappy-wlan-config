...
func (config *Controller) Export() {
	...
	for interfaceName, networks := range interfaces {
		...
		accesspointConfigPath := config.GetAccesspointConfigPath()
		Trace.Printf("create %v", accesspointConfigPath)

		accesspointConfigFile, err := os.Create(accesspointConfigPath)
		if err != nil {
			Warning.Fatalf("create %v : %v", accesspointConfigPath, err)
		}
		defer accesspointConfigFile.Close()
		...
		for _, network := range networks {
			...
			accesspointConfigFile.Seek(0, 0)

			// add a file header
			fmt.Fprint(accesspointConfigFile, "# DO NOT CHANGE THIS FILE\n")
			fmt.Fprint(accesspointConfigFile, "# This file is generated by snappy-wlan-config,\n")
			fmt.Fprint(accesspointConfigFile, "# manual changes may become reversed.\n")
			fmt.Fprint(accesspointConfigFile, "\n")

			// add each network configuration
			export := NewAccesspointExport(&network)
			fmt.Fprint(accesspointConfigFile, export.Dump())
			fmt.Fprint(accesspointConfigFile, "\n")
			accesspointConfigFile.Sync()
		}
